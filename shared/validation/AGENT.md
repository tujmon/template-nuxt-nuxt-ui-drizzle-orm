...
- [auth.ts](shared/validation/auth.ts): Validações para formulários de login, registro e atualizações cadastrais.
- [performance.ts](shared/validation/performance.ts): Validação de métricas recebidas pelo endpoint de telemetria.
