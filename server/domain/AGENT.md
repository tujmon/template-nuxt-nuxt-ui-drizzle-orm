...
- **user/**: Lógica de domínio do recurso de usuário:
  - [entity.ts](server/domain/user/entity.ts): Modelo puro de entidade `UserDomain` e funções auxiliares de validação de domínio.
  - [repository.ts](server/domain/user/repository.ts): Interface (port) definindo operações de persistência necessárias para a lógica do domínio.
