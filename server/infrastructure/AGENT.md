...
- **repositories/**: Implementações concretas das interfaces de repositório definidas na camada de Domínio:
  - [user.repository.ts](server/infrastructure/repositories/user.repository.ts): Implementação de `UserRepository` (`DrizzleUserRepository`) usando queries Drizzle ORM.
